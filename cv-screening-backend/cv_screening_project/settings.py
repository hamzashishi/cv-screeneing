...
ALLOWED_HOSTS = [host.strip() for host in config('ALLOWED_HOSTS', default='localhost,127.0.0.1,cv-screening-backend.onrender.com,cv-screeneing-8.onrender.com,.onrender.com').split(',') if host.strip()]
for required_host in ['cv-screeneing-8.onrender.com', '.onrender.com']:
    if required_host not in ALLOWED_HOSTS:
        ALLOWED_HOSTS.append(required_host)

CSRF_TRUSTED_ORIGINS = [origin.strip() for origin in config('CSRF_TRUSTED_ORIGINS', default='https://cv-screening-ten.vercel.app,https://cv-screening-backend.onrender.com,https://cv-screeneing-8.onrender.com').split(',') if origin.strip()]
for required_origin in ['https://cv-screeneing-8.onrender.com']:
...
CORS_ALLOWED_ORIGINS = config('CORS_ALLOWED_ORIGINS', default='http://localhost:3000,http://localhost:3001,https://cv-screening-ten.vercel.app,https://cv-screening-backend.onrender.com,https://cv-screeneing-8.onrender.com').split(',')
